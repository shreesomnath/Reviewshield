...
def review_one(gen, judge, variant: dict, generate_fn, defense: str | None) -> dict:
    ...
    text = generate_fn(gen, prompt, max_new_tokens=EVAL_MAX_NEW_TOKENS, do_sample=False)  # explicit: frontier_generate defaults to do_sample=True/temp=0.7, which silently made past Gemini eval runs non-deterministic (see content_capture.py for the same fix applied earlier)
