"""SUPERSEDED - kept for provenance only, do not run this to reproduce
paper numbers. Reads Gemini D0/D1 from eval/, which was generated with
the frontier backend's do_sample left at its client default (True,
temperature=0.7) -- non-deterministic, unlike every other reported
number in the paper. The corrected version, reading from eval_gemini_fix/
(generated by evaluation/run_eval_gemini_combined.py with do_sample=False),
is scripts/analysis/plot_gemini_vs_trained.py.
...
def main():
    ...
    fig.tight_layout()
    out_path = FIG_DIR / "gemini_vs_trained_qwen.png"
